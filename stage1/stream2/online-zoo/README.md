...
#### Проверяем на резиновый дизайн (Responsive) и снимаем баллы (но не более -30), если нарушены следующие пункты:
...
#### Проверяем на резиновый дизайн (Responsive) и снимаем баллы (но не более -20), если нарушены следующие пункты:
Пошагово выполняем следующие действия:
1. Растягиваем страницу на ширину больше 1600px, либо уменьшаем масштаб.
2. Изменяем ширину страницы до размеров от 1599px до 1001px.
3. Изменяем ширину страницы до размеров от 999px до 641px.
...
- При открытии появляется меню, как на дизайне. Однако, на дизайне не хватает "крестика" (кнопки закрытия) в правом верхнем углу, поэтому ее нужно добвать на свой вкус. Можно взять из дизайна попапа отзыва.  
...
- При открытии появляется отзыв, как на дизайне, а также область вокруг отзыва как в дизайне, крестик в правом верхнем углу.  
...
1. Открываем на ширине экрана 640px или более так, чтобы основное меню навигации было скрыто, и было видно бурегр-меню. Если бургер меню открывается, ставим **+10**. Если меню по нажатию не открывается - ставим **0** за этот пункт и переходим к следуюшему. Баллы снимаем, если:
...
3. Открываем на ширине экрана 1000px или более так, чтобы были видны 6 карточек животных, и стрелки были между верхним и нижним рядом. Проверяем условия из пункта 1. Они должны выполняться для двух рядов так, как будто бы это был один ряд.
4. Открываем на ширине экрана 640px или более так, чтобы были видны 4 карточки животных, и стрелки были между верхним и нижним рядом. Проверяем условия из пункта 1. Они должны выполняться для двух рядов так, как будто бы это был один ряд. Проверяем также на точность анимации: сдвиг не на 6 карточек, а на 4.  
...
- При количестве интервалов от 1 до 7, а также 10 и больше: **-5**.
...
2. Открываем на ширине экрана 1000px или более так, чтобы были видны 3 отзыва, вместо 4х. Проверяем условия из пункта 1. ❗За отсутствие анимации повторно баллы не снимаем!  
  
**Попап** при нажатии на отзыв в блоке `Testimonials`. 
1. Открываем на ширине экрана 640px или более так, чтобы были видны 3 укороченных отзыва. Если попап появляется целиком, ставим **+10**. Если по нажатию отзыв ничего не происходит (отзыв не открывается) - ставим **0** за этот пункт и переходим к следуюшему. Баллы снимаем, если:
- Отсутствует внешняя область или крестик: **-5**. А так же учитыватся пункт снизу.
...
2. Открываем на ширине экрана 1000px или более так, чтобы были видно максимальное значение Amount в 2000$. Проверяем условия из пункта 1.
3. Открываем на ширине экрана 640px или более так, чтобы были видно максимальное значение Amount в 500$. Проверяем условия из пункта 1.
