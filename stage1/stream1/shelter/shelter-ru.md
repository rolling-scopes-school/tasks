...
Первое, что нужно сделать - это сделать дубляж макета в фигме. Для этого вверху экрана нажимаем на имя проекта, расположенное рядом с надписью "view only". Далее выбираем "Duplicate to your drafts", что создаст вам локальную копию, с которой вы сможете работать.
...
- Логотип находится слева. Логотип состоит из текстовых элементов.
...
1. **Header** (`<header>` содержит только логотип и панель навигации)
- Интерактивная панель навигации. По умолчанию должен быть подсвечен элемент `Our pets`.
