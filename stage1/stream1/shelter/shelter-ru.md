...
## Порядок оценки cross-check неделя 3 🚧 Условия в разработке
...
Если *бургер меню* реализовано только на одной из страниц, а на второй отсутствует, то ставим **0**. Иначе, если меню открывается и закрывается, ставим **+20**. Далее проверяем на условия на каждой странице независимо:
- Бургер меню будет на странице только при width < 768px.
...
- При открытии меню, сама иконка меню поворачивается на 90 градусов и присутствует анимация поворота иконки (rotate).
- После открытия вертикальный скролл должен стать неактивным. Однако, скролл может быть в самом меню, если по высоте оно меньше, чем окно браузера.
...
- При нажатии вне границ меню, на затемненную область, или на кнопку с иконкой бургер меню, меню заезжает обратно.
- При закрытии присутствует анимация заезда (slide-out).
- При закрытии меню, сама иконка меню поворачивается обратно на 90 градусов и присутствует анимация поворота иконки (rotate).
- После закрытия вертикальный скролл должен стать вновь активным.
- При нажатии на "About the shelter" страница окажется в начальном положении и меню будет закрыто.
- При нажатии на "Contacts" нас перебросит в низ страницы, к блоку *Footer* и меню будет закрыто.

За невыполнение любого из этих пунктов, снимаем **-3** балла.

----

#### Popup

Если *попап* реализован только на одной из страниц, а на второй отсутствует, то ставим **0**. Иначе, если при нажатии на карточку животного открывается попап, а при нажатии на крстик (Х) закрывается, ставим **+15**. Далее проверяем на условия на каждой странице независимо:
...
За невыполнение любого из этих пунктов, кроме последнего, снимаем **-3** балла.
...
Если при нажатии на кнопки *влево* и *вправо* происходит перелистывание слайдов, ставим **+25**. Далее проверяем на условия:
- Слайдер реализован со стрелакми, при нажатии на которые случается переход на новый блок элементов.
- Присутствует анимация карусели для смены слайдов. Время анимации и тайминг функция значения не имеют.
...
- Каждый новый слайд будет содержать **псевдослучайный** набор карточек животных, т.е. формироваться из исходных объектов в случайном порядке, но с условиями:
...
- Делаем три проверки на трех разных значениях ширины окна браузера, при этом перезагружая страницу:
...
За невыполнение любого из этих пунктов или подпунктов, снимаем **-5** баллов.
...
Если при нажатии на кнопки *влево* и *вправо* происходит перелистывание страниц с карточками животных, ставим **+40**. Далее проверяем на условия:
...
- Делаем три проверки на трех разных значениях ширины окна браузера, при этом перезагружая страницу:
- - При 1280px <= width на странице одновременно показаны 8 карточек питомцев, а самих страниц 6. Т.е. при нажатии `>>` открывается шестая страница.
- - При 768px <= width < 1280px на странице одновременно показаны 6 карточек питомцевв, а самих страниц 8. Т.е. при нажатии `>>` открывается восьмая страница.
- - При width < 768px на странице одновременно показаны 3 карточки питомцев, а самих страниц 16. Т.е. при нажатии `>>` открывается шестнадцатая страница.
- Каждая новая страница пагинации будет содержать **псевдослучайный** набор питомцев, т.е. формироваться из исходных объектов в случайном порядке, но с условиями:
