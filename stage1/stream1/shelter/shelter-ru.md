...
- на этом этапе вам предстоит адаптировать согласно макету ранее созданные страницы под различную ширину окна от 1280px до 320px включительно.  
...
- Кнопки должны быть интерактивными. Макет содержит отдельно спроектированные блоки с описанием того, как выглядит кнопка с hover-эффектом и без него.
...
- Логотип находится слева. Логотип состоит из текстовых элементов. Нажатие логотип оставляет нас на текущей странице.
...
- Карточки с питомцами должны быть интерактивными при наведении на любую область карточки. Наведение на карточку может только менять курсор, а может и делать подсветку кнопки `Learn more`, или даже менять фон.
...
- Нажатие на `Contacts` перебрасывает нас на блок *Footer*, расположенный на этой же странице (якорная ссылка).
- Логотип находится слева. Логотип состоит из текстовых элементов. Нажатие логотип перебрасывает нас на страницу *main*.
- На странице обязательно должен присутствовать один элемент `<h1>`. Можно сделать его с текстом `Cozy House`.
...
- Фон сильно отличается от того, что на макете (не имеется ввиду смещенная картинка или градиент): **-1**.
- Нет картинки собаки: **-2**. Картинка собаки есть, но сильно смещена, или накладвается на текст или другие блоки: **-1**.
  
3. Нет блока **About**: **-5**.
- Нет картинки кошки и собаки: **-2**. Картинка есть, но сильно смещена, или накладвается на текст или другие блоки: **-1**.
...
- Наведение на карточку может только менять курсор, а может и делать подсветку кнопки `Learn more`, или даже менять фон. При это будут сниматься баллы, если: 
- Карточки не интерактивные: **-2**.
- Кнопки `Learn more` не интерактивные: **-2**.
...
- Кнопки "влево" инетарактивные: **-1**.
- Кнопки "вправо" не инетарактивные: **-1**.
- На кружочке страницы нет цифры, или там не единица: **-1**.
- Кружочек страницы инетарактивный: **-1**.
- Нет карточек с питомцами: **-10**. Карточки есть, но при этом:
- Сеточная структура элементов нарушена: **-2**.
- Количество карточек не соотвествует макету: **-2**.
- Струтура карточек нарушена (например текст или кнопка сверху над картинкой): **-2**.
