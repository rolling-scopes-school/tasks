...
### Дополнительные требования к репозиторию
...
- **[Дополнительные требования к репозиторию](#дополнительные-требования-к-репозиторию)**
...
❗ **Важное замечание #1**: При изменении ширины с использованием инструментов разработчика (dev tools) и наличии вертикального скролла расхождение может составлять до 17px. В таком случае надо либо изменить ширину на 17px, либо перейти в режим просмотра страницы на устройстве, чтобы скролла не было.  
❗ **Важное замечание #2**: При масштабировании (например, zoom+ 125%) реальная ширина может отличаться на 1-2 пикселя. Например, реальное значение может быть 767 или 769, хотя инструменты разработчика будут показывать 768. Поэтому надо сдвинуть до точки перехода, несмотря на отличие.  
❗ **Важное замечание #3**: Единственненный блок, в котором элементы не обязательно должны соблюдать структуру - это блок `How you`. Если элементы идут как перечесляемые элементы, то при проверке на резиновую верстку, отступы от края блока до элементов, а также количество в ряду не учитвыются и баллы не снижаются. Также не оценвается, центрируются ли элементы или прибиты к левому краю. Однако нарушение размеров учитывается, будьте внимательны!

----

❗ Ознакомьтесь с некоторыми определениями: 
- Нарушение отступов - это ситуация, когда расстояние между элементами, или расстояние от элемента до края контейнера в котором он содержится, отличается от заданного на макете белее, чем на 10 пикселей по высоте или ширине. Рассмотрим ситуацию с блоком `our friends`. Вы, скорее всего, будете знать примерное расстоние между элементами на глаз. Однако, вас смущает отступ кнопки "влево" от края экрана слева. Расстояние от кнопки "влево" до левой границы блока 40px на ширине экрана 1280px. Значит, выставив ширину 1280px, смотрим расстояние. Если оно отличается на величину до 10px (например 50px или 30px) - баллы не снимаем. Однако, если отступы больше этих значений (например 51px или 29px) - снимаем указанное количество баллов.
- Нарушение раззмеров - это ситуация, когда размеры элементов (кнопок, картинок, карточек животных, иконок) отличаются от заданного на макете белее, чем на 10 пикселей по высоте или ширине. Проверку проводим таким же образом, как и при нарушении отступов, и если отличия большие, снимаем указанное количество баллов.
- Длину текста и слов по ширине, а также порядок переноса слов по строкам, мы не учитываем при проверке.
- Уникальная ошибка - это новая найденная ошибка, которая, при этом, может повторяться для однотипных элементов. Например, съехала кнопка во всех карточках животных. Снимать баллы будем лишь один раз, т.к. скорее всего сломан общий стиль. Однако, если карточка со съехавшей кнопкой одна - снимаем также указанное количество баллов. Т.е. нет разницы, ошибка в одном повторяющемся элементе, или в нескольких.


----

Хоть при растягивании экрана на 4k, хоть при уменьшении до 320px не должна теряться целостность приложения. При плавном изменении размера, мы не учитываем точных подсчетов расположения элементов. Однако, в первую очередь, не должен появляться горизонтальный скролл. Во-вторых, элементы ни при каких обстоятельствах не должны начать вылазить за рамки экрана. Если, при проверке в браузере **Google Chrome** сайт ведет себя отзывчиво, элементы и общая структура меняет размеры вместе с изменением ширины, не появляется горизонтальная полоса прокрутки, то ставим **+50** за каждую страницу. В сумме за 2 страницы ставим **+100**. Если на странице появляется горизонтальная полоса прокрутки, ставим **0** за страницу!
  
----

При проверке в браузере **Mozilla Firefox**, если, при изменении размера окна браузера от максимума до 320px появляется горизонтальная полоса прокрутки, или контент вылазит за пределы экрана, снимаем **-20**. Если же появляются новые проблемы, отличные от тех, что были в браузере Google Chrome, снимаем **-5** за каждую, и описывам в фидбеке, но суммарно не более **-20**.
  
----

### 1280px <= width

При проверке на больших экранах, допустимо либо, что блоки продолжают свой цвет на всю доступную область окна, а сама обертка центрируется. Либо, что макет занимает максимальную ширину в 1280px и центрируется с равными отступами справа и слева, любого цвета. Ни то, ни другое не считается ошибкой. Однако, если при ширине более 1280 макет прижат либо к левой, либо к правой стороне, в то время как с другой стороны пустое пространство - это ошибка, снимаем **-10** за каждую страницу.

В случае, если макет на данной ширине резиновый, проверяем условия ниже.
...
Если условия нарушены, снимаем **-5** за каждый пункт.  
Если блоки или элементы выходят за рамки экрана, или накладываются друг на друга, снимаем **-5** за каждую уникальную ошибку.  
Выставляем ширину в 1280px. За нарушение отступов или нарушение размеров снимаем **-3** за каждую уникальную ошибку.  
Суммарно можем снять не более **-10**.
...
Если условия нарушены, снимаем **-5** за каждый пункт.  
Если блоки или элементы выходят за рамки экрана, или накладываются друг на друга, снимаем **-5** за каждую уникальную ошибку.  
Выставляем ширину в 768px. За нарушение отступов или нарушение размеров снимаем **-3** за каждую уникальную ошибку.  
Суммарно можем снять не более **-15**.
...
Если условия нарушены, снимаем **-5** за каждый пункт.  
Если блоки или элементы выходят за рамки экрана, или накладываются друг на друга, снимаем **-5** за каждую уникальную ошибку.  
Выставляем ширину в 320px. За нарушение отступов или нарушение размеров снимаем **-3** за каждую уникальную ошибку.  
Суммарно можем снять не более **-15**.
...
- В слайдере активные элементы выделяются более светлым цветом заднего фона, и реагируют на `hover`.
