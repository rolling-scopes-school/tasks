...
(`AGENTS.md` + `LOG.md`) ships in a later iteration; until then, follow these rules manually and keep a
`LOG.md` yourself.
...
- **Decisions are yours.** At each **decision point** below, have the AI lay out the options with
  pros/cons, then **you choose and write down why** in `LOG.md`.
- **`LOG.md` is mandatory.** Keep an append-only log: what you did each session, which decisions you
...
- Vite `react-ts` project with **Oxlint**, **Prettier**, and **Husky** (pre-commit: lint+format).
...
2. Stack: **Vite + React + TypeScript + Oxlint + Prettier + Husky + Vitest + RTL** (+ MSW optional).
3. Decompose the UI into components (e.g., `SearchBar`, `WeatherCard`, `SavedLocations`, `ErrorBoundary`).
4. Keep a `LOG.md` as described in "Working with AI".
