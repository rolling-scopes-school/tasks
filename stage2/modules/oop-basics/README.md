...
&nbsp;&nbsp;&nbsp;&nbsp; С момента появления Объектно ориентированное программирование (ООП) было и остается одной из самых популярных 
парадигм, которая позволяет писать модульный расширяемый код с использование хорошо зарекомендовавших
себя практик. 
...
отмерь, один раз отрешь? Это как раз оно. Принцип интуитивно понятный и я настоятельно рекомендую им пользоваться всегда. 
...
Нет. Автомобиль будет автомобилем только, когда мы соберем его в одно единое целое из составных частей. Это и есть 
композиция.
...
&nbsp;&nbsp;&nbsp;&nbsp; Полиморфизм также пришел в программирование из реальной жизни. Обернитесь вокруг, на каждом шагу вы увидите объекты, которые
...
Когда мы наследуемся от какого-то класса или имплементируем какой-либо интерфейс - мы обязаны реализовать все его методы, взамен мы можем быть уверены что поведение программы не будет меняться даже при том что используются разные типы объектов.
