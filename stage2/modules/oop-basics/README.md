...
или если по-русски: сначала думаем, потом делаем. В русском языке много поговорок на эту тему например: Семь раз 
отмерь, один раз отрешь. Принцип интуитивно понятный и я настоятельно рекомендую им пользоваться всегда. 
unior разработчики как правило - очень порывистые люди:

![junior](images/junior.png)

&nbsp;&nbsp;&nbsp;&nbsp; Что иногда может сыграть с ними злую шутку, так что в перую очередь, на старте вашей успешной карьеры возьмите
...
и продумать за что должны отвечать и какие методы и свойства дола нести каждая из них.
...
взять за правило, сначала хотя бы строить диаграмму, и только после этого начинать разработку. Это позволит
...
отрефакторить весь написанный для этого код. Это всегда съедает очень много времени и ДЕНЕГ заказчика и может
отрицательно сказаться на вашей карьере. 

Наиболее удобные инструменты для построения диаграмм - новый функционал в [Figma](https://www.figma.com/)
...
в служебно классе Array. Таким образом мы получаем все возможности массива без написания нового массива. Это и есть 
композиция.
