...
&nbsp;&nbsp;&nbsp;&nbsp; С момента появления Объектно ориентированное программирование (ООП) было и остается одной из смых популярных 
парадигм, которая позволяет писать модульный расширяемы код с использование хорошо зарекомендовавших
себя практик. 
 
&nbsp;&nbsp;&nbsp;&nbsp; ООП включает в себя принципы, которые позволяют сократить дублирование кода к минимуму, 
...
отмерь, один раз отрешь? Это как рза оно. Принцип интуитивно понятный и я настоятельно рекомендую им пользоваться всегда. 
Junior разработчики как правило - очень порывистые люди:

![junior](images/junior.png)

&nbsp;&nbsp;&nbsp;&nbsp; Что иногда может сыграть с ними злую шутку, так что в первую очередь, на старте вашей успешной карьеры возьмите
этот принцип на вооружение. 

&nbsp;&nbsp;&nbsp;&nbsp; Но так как же это все связанно с абстракцией? Ответ - напрямую. До того как вы напишите
...
а так же инструменты, которые позволяют удобно с ним работать. Стоит ли много времени уделять UML диаграммам?
Нет, достаточно просто запомнить что это язык построения диаграмм, базово запомнить принцип их построения, а далее
взять за правило, сначала хотя бы базово строить диаграмму и хоть минимально подумать о том как части системы 
...
упрощенной, тем не менее она должна решать какую-то задачу. В нашем случае это перемежение объекта пассажир из одного 
...
Самостоятельная работа - добавьте к диаграммам пилотов и борт проводника. Пилоты должны управлять самолетом,
борт проводники должны помогать пассажирам.

## Наследование

Дублирование кода это плохо. Дублирование кода, как правило, является самым простым способом решения задачи в лоб, но оно
всегда приводит разбуханию кодовой базы и увеличению сложности программы, а это в свою очередь не позволяет
...
придерживаться принципу [Don't Repeat Yourself(DRY)](https://ru.wikipedia.org/wiki/Don%E2%80%99t_repeat_yourself) - не
повторяйтесь. 
...
Само наследование как термин вам должно быть из биологии, это как раз возможность биологических организмов передавать
...
аэропорт и еще три одинаковых классов по одному для каждого типа. Получается что на каждое N добавление типа нам нужно 
...
людей (борт проводников и пилотов) есть общие поля, как минимум имя и id. Плюс к тому, все они люди. Зачем нам одинаковые 
поля в нескольких классах? Давайте поправим это с помощью наследования! Вперед! За дело!

## Композиция

&nbsp;&nbsp;&nbsp;&nbsp; Композиция это мощный инструмент расширения функционала объекта. Он так же знаком вам из реальной жизни. Как пример - стол за которым
...
&nbsp;&nbsp;&nbsp;&nbsp; Посмотрите внимательно на диаграмму которую мы создали выше, в частности н Аэропорт 
и Самолет. Они содержат массивы, но сами методы массивов не нужно реализовывать в этих классах, ведь они уже реализованны
в служебно классе Array. Таким образом мы получаем все возможности массива без написания нового массива. Сложно? 
Думаю для новичков это действительно может быть сложно для понимания. Давайте пример по проще. Все мы видели например 
...
свои свойства, не свойственные другим деталям автомобиля. Но можем ли мы назвать допусти колесо автомобиля, автомобилем?
Нет. Автомобиль будет автомобилем только, когда мы соберем его в одно единое целом из составных частей. Это и есть 
композиция.
...
&nbsp;&nbsp;&nbsp;&nbsp; Полиморфизм также пришел в программирование из реальной жизни. Обернитесь вокруг, на каждом шагу вы увидите объекты которы
несмотря на разные типы можно использовать одинаково. Например любое устройство можно подключить к стандартной розетки через
...
Инструменты полиморфизма:
 ...
 Когда мы наследуемся от какого-то класса или имплементируем какой-либо интерфейс - мы обязаны реализовать все его методы,
 взамен мы можем быть уверенны что поведение программы не будет меняться даже при том что используются разные типы объектов.
 Это является одним из важнейших свойств которое используется во многих паттернах проектирования.
  ...
  А так же ответьте на вопрос. Является ли полиморфизмом возможность любых самолетов, вне зависимости от типа садиться в 
  аэропортах?
...
инкапуслированный интерфейс выступает поддержкой для остальных принципов, так как от хорошо инкапсулированного объекта
значительно проще отнаследоваться. 

&nbsp;&nbsp;&nbsp;&nbsp; Но давайте вернемся к примеру с автомобилем, вы же понимаете что устройство 
автомобиля намного сложнее? Что было бы если бы рядовому пользователю был доступно регулирование впрыска топлива в двигатель? 
...
но большая часть пользователей с большой вероятностью в короткий срок вывели бы свой автомобиль из строя. Все это элементы
которые опасны для обычного пользователя, а значит они должны быть от него сокрыты.  
...
должен уметь: 
  ...
  - тормозные колодки которые долбни уметь сжиматься
...
[короткая статься на tproger](https://tproger.ru/translations/oop-principles-cheatsheet/)
[неплохая статься на хабре](https://habr.com/ru/post/463125/)