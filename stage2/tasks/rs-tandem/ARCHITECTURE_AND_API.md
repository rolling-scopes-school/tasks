...
Поэтому мы предлагаем использовать паттерн API Service Layer (частный случай паттерна Repository).
...
Вместо того чтобы UI-компонент знал _как_ получить данные (fetch, SQL, localStorage), он просто _просит_ их у Repository.
...
_Стек: Supabase, Firebase, Appwrite._
...
_Стек: Node.js + Express / NestJS + PostgreSQL / Mongo._
