...
В этом проекте мы дали вам много свободы выбора: что именно делать и как. Поэтому, чтобы мы могли понять ваш личный прогресс, мы просим вас вести дневник разработки. В реальных проектах это может встречаться в виде официального Architecture Design Log, но в нашем случае это будет что-то более простое и не настолько формальное.
...
## Требование: 2 личных компонента

В этом проекте разрешено использовать AI для написания кода. Однако **минимум 2 компонента** должны быть написаны **лично вами**.

**Почему это важно:**
Мы хотим убедиться, что вы понимаете код, который сдаёте. AI — отличный помощник, но финальный проект должен демонстрировать *ваши* навыки, а не навыки LLM.

**Что это значит на практике:**
- В дневнике должно быть описание разработки **минимум 2 компонентов**
- Для каждого компонента: какие проблемы возникли, как решали, что изучили
- AI может помогать (подсказки, генерация бойлерплейта), но основная логика — ваша работа
- Это подтверждается историей коммитов и записями в дневнике

**Пример структуры для компонента:**
```markdown
## Компонент: Quiz Widget

**Что делает:** Интерактивный виджет с вопросами и drag & drop.

**Мой вклад:**
- Архитектура состояния (useState vs useReducer — выбрал useReducer, потому что...)
- Логика валидации ответов
- Анимации перехода между вопросами

**AI помогал с:**
- Бойлерплейт для drag & drop библиотеки
- CSS стили для анимаций

**Проблемы и решения:**
- Проблема: Порядок элементов сбивался при быстром перетаскивании
- Решение: Добавил debounce и оптимистичный UI update
```

**Без выполнения этого требования:**
- Личная оценка = 0 баллов
- Допуск к защите не предоставляется
...
7. **Совет:** Добавьте напоминание в свой календарь, что надо потратить 15 минут на ведение дневника.
...
## Критерии оценки Development Diary (Max 50 points)
...
| Требование | Статус |
|------------|--------|
| Дневник существует и заполнен регулярно | must pass |
| **Описаны минимум 2 личных компонента** | must pass |
| Записи закоммичены своевременно (не в последний день) | must pass |

**Если порог не пройден → 0 баллов за личную оценку, допуск к защите не предоставляется.**

### Критерии качества (0-50 баллов)

| Баллы | Уровень | Описание |
|-------|---------|----------|
| **0** | Нет | Дневника нет или он фиктивный (залит в последний день) |
| **10** | Формальный | Краткие записи без деталей ("сделал X") |
| **20-30** | Регулярный | Минимум 2 записи/неделю, виден прогресс, описаны 2 компонента |
| **40** | Детальный | Есть описание проблем и решений, понятен ход мысли |
| **50** | Рефлексивный | Глубокая рефлексия, Post-Mortem анализ, сравнение вариантов решений |