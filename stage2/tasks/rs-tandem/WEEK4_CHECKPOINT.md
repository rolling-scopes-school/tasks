...
> **Score в RS APP:** Tandem: Week 4 Checkpoint
> **Тип оценки:** личная (каждый студент получает индивидуально)
> **Дедлайн:** 16 марта 2026, 23:59 UTC
> **Оценка:** до 20 баллов (10 за тесты + 10 за code review)

## Суть задания

Две задачи: **написать тесты** и **провести code review**.

Напишите минимум 5 unit-тестов для кода, который вы лично разработали. И разберитесь в коде другого участника команды — оставьте комментарии в PR или опишите в дневнике, как вы изучали чужой код.

---

## Условия получения

### Часть 1: Unit Testing (10 баллов)

**Все условия выполнены → 10 баллов, любое не выполнено → 0 баллов.**

| #   | Условие                                  | Детали                                                                                                   |
| --- | ---------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| 1   | **Тестовый фреймворк установлен**        | `vitest`, `jest` или аналог в `devDependencies` файла `package.json`.                                    |
| 2   | **`npm test` работает**                  | Команда `npm test` выполняется без ошибок и запускает тесты.                                             |
| 3   | **Минимум 5 тестов написаны лично вами** | Файлы `*.test.*` или `*.spec.*` в проекте. Суммарно ≥5 тестовых кейсов (`it` / `test`) в ваших коммитах. |

### Часть 2: Code Review (10 баллов)

**Все условия выполнены → 10 баллов, любое не выполнено → 0 баллов.**

| #   | Условие                          | Детали                                                                                                                                                                                                                                          |
| --- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 4   | **Разобрались в коде тиммейта**  | **Вариант А:** ≥3 содержательных комментария в PR команды (не "ок" / "lgtm"). **Вариант Б:** Запись в дневнике о том, как вы изучали код другого участника — что разобрали, что поняли, какие артефакты остались (заметки, вопросы, скриншоты). |
| 5   | **Ссылки / описание в дневнике** | В записи за Week 4 — ссылки на PR с вашими комментариями (Вариант А) или описание процесса изучения кода (Вариант Б).                                                                                                                           |

### Общее (обязательно для обеих частей)

| #   | Условие                                   | Детали                                                                                |
| --- | ----------------------------------------- | ------------------------------------------------------------------------------------- |
| 6   | **Минимум 2 записи в дневнике за Week 4** | Записи с датами **после 9 марта**, замержены в `main`. Без дневника — 0 за обе части. |

---

## Часть 1: Unit Testing

### Что тестировать

Тестируйте код, который вы **лично написали**. Вот идеи от простого к сложному:
...
Оставьте ≥3 содержательных комментария в Pull Request(ах) вашего командного репозитория. Можно в одном PR или в нескольких.

Комментарий по существу кода. Три категории, которые засчитываются:

**1. Предложение тестов** — самый простой вариант. Посмотрите на код в PR и предложите, что стоит протестировать:
...
### Как оформить в дневнике

**Для Варианта А** — секция со ссылками на PR:
...
**Для Варианта Б** — описание процесса изучения кода:
...
## Можно ли зачесть ранее написанные тесты и ревью?

Да — при двух условиях:

1. **Тесты написаны и ревью проведено лично вами** (не сгенерированы AI целиком).
2. **Работа отражена в дневнике за Week 4** — ссылки на PR, упоминание тестов.

Если вы уже писали тесты на Week 2-3 — отлично. Укажите в дневнике, что тесты были написаны ранее.
...
| Вопрос                                         | Ответ                                                                                                                                                    |
| ---------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Можно использовать Jest вместо Vitest?         | Да. Любой тестовый фреймворк для JS/TS.                                                                                                                  |
| Нужны ли E2E тесты?                            | Нет. Достаточно unit-тестов. E2E — опционально для Personal Features.                                                                                    |
| 5 тестов — это мало, зачем?                    | Цель — начать. 5 тестов = привычка сформирована. Дальше дописать легче, чем начать с нуля.                                                               |
| Можно ли ревьюить PR вне своей команды?        | Нет. Только PR в вашем командном репозитории.                                                                                                            |
| Что если в команде нет открытых PR?            | Используйте Вариант Б — опишите в дневнике, как разбирались в коде тиммейта. Или создайте PR со своими тестами и попросите тиммейта сделать то же.       |
| Можно ли использовать AI для написания тестов? | AI может помочь разобраться с синтаксисом, но тесты должны быть осмысленными и покрывать ваш код. Копипаста из ChatGPT без понимания — не засчитывается. |
...
Отдельный сабмит не требуется. Парсер проверяет: наличие тестового фреймворка в `package.json`, тестовые файлы в проекте, ссылки на PR в дневнике. Оценка выставляется после дедлайна.
