...
В [SCORE_PERSONAL.md](SCORE_PERSONAL.md) за **явное и обоснованное применение паттернов** и за **выделение API Layer** начисляются баллы (+10 за каждый). Ниже — паттерны, которые прямо полезны в примерах Tandem (Widget Trainer, Codenames, AI Prep App) и которые решают типичные проблемы проекта.
