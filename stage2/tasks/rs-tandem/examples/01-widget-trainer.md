...
- **Валидация (Кнопка "Collect"):**
  - Если помечен объект, до которого _можно_ добраться от Root → **Ошибка** ("Удалили живой объект!").
  - Если не помечен объект, до которого _нельзя_ добраться → **Ошибка** ("Memory Leak! Вы пропустили мусор").
