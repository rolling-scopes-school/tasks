...
| Boris (Firebase-Dev) | Firebase-Dev | >= 15 вопросов  | Конец недели 3 |
...
| -------------------------------------------- | ------- | ------- | ------- | ----------------- | ------------------------------------------ |
| Popup UI (HTML/CSS, модальное окно, overlay) | 3ч      | 5ч      | 4ч      | Diana (Check-Dev) | Анимация входа/выхода                      |
| Self/Peer evaluation flow                    | 3ч      | 6ч      | 4.5ч    | Diana (Check-Dev) | "Показать ответ" -> оценка                 |
| AI evaluation flow (мок-интеграция)          | 3ч      | 5ч      | 4ч      | Diana (Check-Dev) | Подключение к AICheckEvaluator на хосте    |
| Question Bank: структура + 20 вопросов       | 3ч      | 6ч      | 4.5ч    | Diana (Check-Dev) | Начальный контент                          |
| Check-таймер (30 сек) интеграция             | 2ч      | 4ч      | 3ч      | Diana + Boris     | Отдельный таймер, checkDeadline в Firebase |
| Firebase-интеграция (пути, onValue, push)    | 2ч      | 4ч      | 3ч      | Diana (Check-Dev) | rooms/{code}/check, rooms/{code}/actions   |
| Анимации (popup enter/exit, результат)       | 1ч      | 3ч      | 2ч      | Diana (Check-Dev) | CSS transitions                            |
...
- **Diana (Check-Dev)** — ответственный за попап, flow и peer review контента
- **Boris (Firebase-Dev)** — помогает с Firebase-путями и Security Rules
