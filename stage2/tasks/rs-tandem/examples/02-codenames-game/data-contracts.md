...
> **Важно:** Все AI интерфейсы реализованы через моки. Реальная интеграция с LLM — отдельная задача, которая подключается через тот же интерфейс без изменения остального кода.
...
src/
├── api/
│   ├── types.ts                 # Все типы из этого документа
...
│   ├── ws-events.ts             # ClientEvent, ServerEvent типы
│   ├── mock-ws-client.ts        # Mock WS client
│   ├── real-ws-client.ts        # Real WS client (Socket.IO)
│   └── index.ts                 # Переключение по env
├── ai/
│   ├── ai-spymaster.ts          # AISpymasterService interface
│   ├── ai-check-evaluator.ts    # AICheckEvaluatorService interface
...
│   │   └── auth-middleware.ts   # Проверка токена
│   ├── api/
...
> **Важно:** Без проверки токена любой может подключиться и подменить данные. Настройте авторизацию до 4-й недели.
