...
Студент заполняет копию таблицы фич, отмечая заявленные строки. Файл `self-assessment.md` кладётся в папку `/development-notes/github-username/`. Ментор использует как стартовый чеклист.
...
| **Feature Demo** | 10 мин | Студент показывает свои личные Feature Components. Демонстрирует работу в приложении и показывает код. Ментор отмечает фичи в чеклисте. |
| **Q&A**          | 7 мин  | Ментор задаёт вопросы по коду, архитектуре и по любому чекпоинту (W1-W5). Верифицирует, что студент понимает свой код.                  |
...
2. **Заполнить self-assessment** — файл `self-assessment.md` в папке `/development-notes/github-username/`
3. **Подготовить "О проекте"** — 2-3 предложения: что за приложение, ваша роль в команде
4. **Подготовить демо** своих 2+ Feature Component (знать, что показать и рассказать)
5. **Быть готовым показать код** и объяснить, почему выбрали именно такое решение
6. **Быть готовым к вопросам по чекпоинтам** (W1-W5)
...
3. Посмотреть `self-assessment.md` студента (отмеченные фичи)
