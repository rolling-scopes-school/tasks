...
### 1. Работа с LLM: рекомендации

**Рекомендуем:** Реализовать интеграцию с LLM самостоятельно через `fetch` + `ReadableStream` (или `openai` как HTTP-клиент). Это даёт глубокое понимание того, как работают LLM "под капотом": streaming, токенизация, обработка ошибок.

**Допускается:** Использование высокоуровневых SDK (LangChain, Vercel AI SDK и т.д.).

**Бонус за low-level реализацию:** Если вы реализовали streaming, prompt management и error handling без высокоуровневых SDK — это будет учтено в **Technical Depth** при Mentor Review. Умение разобраться в "базе" ценится выше, чем умение подключить библиотеку.
