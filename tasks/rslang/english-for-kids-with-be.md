...
| ---------------- | ----------- | ----------- |
| 07.12.2020 23:59 | english-for-kids    | english-for-kids    |
...
- все данные для отображения подгружаются с сервера
...
- на страницу администратора можно попасть через кнопку logIn расположенной в сайдбар.
- после нажатия на кнопку, должно открыться окно авторизации с вводом логина и пароля
- после успешной авторизации пользователь попадает в контрольную панель приложения в которой он может:
    - создавать категории
    - удалять категории (все слова связанные с удаленной категорией должны удаляться каскадом)
    - обновлять категории
    - добавлять новые слова к категории
    - изменять названия\аудио дорожку\изображение\перевод для слова в категории
    - удалять слова для категории
...
При загрузке приложения или перезагрузке страницы приложение открывается в режиме тренировки для пользователя без авторизации, 
авторизованный пользователь должен оставаться на странице администратора. Определение авторизации должно быть реализованно 
через token. Переключение между тренировкой и игрой происходит при клике по переключателю Train/Play. 
...
- Angular/React/Vue можно только по согласованию с ментором
...
## Критерии оценки:
**Максимальный балл за задание: 290 баллов при кросс-чеке / 330 баллов при проверке ментором**   
...
- **Backend часть**
  - [ ] реализованна возможность выполнять Create Read Update Delete (CRUD) операций для сущности Word category  (+30)
  - [ ] реализованна возможность выполнять CRUD операций для сущности Word  (+30)
  - [ ] реализованна возможность авторизации на админ страницу.  (+30)
  - [ ] Данные хранятся в базе данных. (+30)
