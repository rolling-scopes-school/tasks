...
- В разделе `Digital Library Cards` реализовать проверку карты пользователя. Если введенные имя и номер карты совпадают с данными пользователя, то отображается панель с информацией, вместо кнопки `check the card` на 5 секунд. После чего кнопка возвращается в прежнее состояние.
#### Этап 2: Пользователь на этапе регистрации
- Нажатие на кнопку `Register` или `Sign Up` открывет модальное окно решистрации. Только после этого можно будет выполнить действие log in. После регистрации, у пользователя изменится иконка пользователя на заглавные буквы имени. Все данные (в том числе и пароль!) сохраняются в localStorage.
#### Этап 3: Пользователь на этапе входа в учетную запись после регистрации
-  При открытом бургер-меню, оно закрвывается и открывается окно регистрации. То же верно и в обратную сторону.
- При нажатии на кнопку Log In появляется окно Login, где есть поля email (or readers card) и Password. Пароль нужно не меньше 66-86 букв. (
...
- **Ограниченная карусель в блоке `About`**
  1. На экране шириной 1440px проверяем, чтобы было доступно 2 других скрытых картинки. При каждом нажатии выезжает следующая, и так до границ справа и слева. +2
  2. Выделенные кнопки становятся неактивными. +2
  ...
  4. На экране шириной 768px проверяем, чтобы было доступно 2 других скрытых картинки. Для этого меняем разрашение и перезагружаем страницу. Теперь доступных перемещений становится 5. +2
  5. Неактивными становятся не только выделенные кнопки, но и стрелочки на границах карусели. +2
  6. 
- **Слайдер в блоке `Favorites`**
  1. На любой ширине экрана все 4 карточки с книгами будут плавно затухать, а затем плавно появляться следующие. +2
  2. Анимация может быть прервана следующим нажатием на кнопку выбора поры года, но при этом анимация не должна застывать в промежуточном состояни. Должна отрабатывать до конца. +2
  3. В случае, если панель управления слайдером сделана по технологии "styicky" для разрешений 768px и меньше, т.е. опускается вниз вместе скроллом страницы, прилипая к верхней части экрана, в рамках блока `Favorites` - это не ошибка и баллы не снимаем.
  4. В самом блоке `Favorites` все кнопки должны иметь имя `Buy`, а не `Own`. +2
- **Карта пользователя В разделе `Digital Library Cards`**
  1. Поле `card number` в форме мдолжно иметь ограничение на ввод 16-ричной системы исчисления. Т.е. это все цифры плюс символы a-f как в большой, так и в малой нотации. +2
  2. Если нажимать кнопку до прохождения
  3. Если введенные имя и номер карты совпадают с данными пользователя (John Doe / F00234030), то отображается панель с информацией, вместо кнопки `check the card` на 5 секунд. +2
  4. После отоюражения информации, кнопка возвращается в прежнее состояние, а поля в форме сбрасываются. +2
### Этап 2: Пользователь на этапе регистрации
- **Меню авторизации при нажатии на иконку пользователя**
  ...
  3. То же верно и в обратную сторону, кнопка бургер-меню должна быть доступна. +2
  4. Нажатие на любую область или элмемент вне меню приводят к закрытию меню авторизации. +2
- **Модальное окно Register**
  1. При нажатии на кнопку Register в открытом меню авторизации появляется модальное окно Register, где есть поля first name, last name, email и Password. +2
  2. При нажатии кнопки Sign Up в блоке  `Digital Library Cards` также появляется модальное окно Register. +2
  3. Окно центрировано, а область вокруг затемнена (насколько затемнена - не имеет значения). +2
  4. При нажатии на крестик в углу окна, или на затемненную область вне этого окна, оно закрывается. +2
  5. В данном случае, ограничения по полям - все поля должны быть не пустыми. +2
  6. Пароль должен быть не короче 8 символов. +2
  7. Поле `Email` имеет паттерн email. +2
...
- **Модальное окно Log In**
  1. При нажатии на кнопку Log In появляется модальное окно Login, где есть поля email (or readers card) и Password. +2
  2. В данном случае, единственное ограничение по полям - пароль должен быть не короче 8 символов. +2
- При нажатии на кнопку Log In появляется окно Login, где есть поля email (or readers card) и Password. Пароль нужно не меньше 66-86 букв. (
### Этап 4: Пользователь после входа в учетную запись
-
