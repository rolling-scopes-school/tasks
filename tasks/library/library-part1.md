...
3. Вёрстка соответствует макету **+54**  
   ...
   - секция `Welcome` +4.
   - секция `About` +6:
     - Добавьте все картинки, которые будут использованы в папку с картинками. Даже если отображается всего 3, в папке должны быть все 5. (Это рекомендация, проверять не нужно)
   ...
   - секция `Favorites` +8:
     ...
     - Картинок и описаний - много, для 4х секций. Их стоит добавить в проект. А лучше сразу на страницу, и скрыть с помощью CSS свойств, например `display: none;`. (Это рекомендация, проверять не нужно)
   ...
   - секция `LibraryCard` +8:
     ...
     - Хоть иконки из модального окна (Visits, Bonuses, Books) сейчас не нужны будут, можно их добавить в соответствующую папку проекта. (Это рекомендация, проверять не нужно)
   - блок `<footer>` +8:
