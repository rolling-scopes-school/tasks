...
5. Общие требования к верстке **+20**
   ...
   - при уменьшении масштаба страницы браузера вся вёрстка (контент) размещается по центру, а не сдвигается в сторону +2. Для этого должна быть обертка вокруг всей страницы, которая выравнивается по центру. Фон (белый) в секциях и тёмный (#0C0C0E) в футере и хедере - тянется на всю ширину экрана. Изображение библиотеки в секции `Welcome` - занимает всю ширину экрана.
