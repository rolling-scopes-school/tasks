...
3. During the work on the project, you need to make at least 3 commits in the `gh-pages` branch. According to the [commit guideline](https://rs.school/docs/git-convention), the name of each commit should start with one of the listed prefixes: `init:`, `feat:`, `fix:`, `refactor:`, `docs:`.
...
7. Create a Pull Request from the `gh-pages` branch to the `main` branch. The title of the Pull Request should be `Markdown & Git`. Follow the [Pull Request description template](https://rs.school/docs/short-track/pull-request-requirements#pull-request-requirements-pr). Do not merge the Pull Request from the `gh-pages` branch to the `main` branch.
