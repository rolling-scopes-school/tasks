...
3. Maintain a commit history during the project. Follow the [commit guideline](https://rs.school/docs/git-convention), where the name of each commit should start with one of the specified prefixes: `init:`, `feat:`, `fix:`, `refactor:`, `docs:`.
...
6. Create a Pull Request from the `rsschool-cv-html` branch to the `gh-pages` branch. The title of the Pull Request should be `HTML, CSS & Git Basics`. Follow the [Pull Request description template](https://rs.school/docs/short-track/pull-request-requirements#pull-request-requirements-pr). Merge the Pull Request from the `rsschool-cv-html` branch to the `gh-pages` branch.
