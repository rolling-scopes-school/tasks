...
6. Create a Pull Request from the `rsschool-cv-html` branch to the `gh-pages` branch. The title of the Pull Request should be `HTML, CSS & Git Basics`. Follow the [Pull Request description template](https://rs.school/docs/pull-request-review-process#pull-request-requirements-pr). Merge the Pull Request from the `rsschool-cv-html` branch to the `gh-pages` branch.
