...
## Критерии оценки 
Необходимо добавить перечень выполненых критериев в readMe файл.
Если есть функциональность приложения которую сложно проверить тогда прикрепляйте ссылку на видио.
Так же для тех кто будет делать бекенд, добавьте в репозиторий папку с бекендом, или ссылку на другой репозиторий.
...
* Понятный, читаемый код. Имена переменных и функций отражают то что в них содержется/то что они делают. Функция выполняет одно действие. Повторение логики сведено к минимуму. (30 баллов)
...
### Дополнительная функциональность приложения/игр:
* Т.к. у студентов есть творческие идеи которые они бы хотели реализовать, предлагаю в качестве эксперимента добавить отдельный пункт, внедрение какой-либо интересной фичи которой нет в данном списке (оценка 20 балов если фича сложная 40 балов)
* В игре проработана "физика" (40 баллов)
* есть сетевой режим игры (80 баллов)
* 3d игра (40 баллов)
