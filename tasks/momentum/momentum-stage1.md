...
4. Виджет погоды +15
   ...
   - выводится уведомление об ошибке при вводе некорректных значений, для которых API не возвращает погоду (пустая строка или бессмысленный набор символов) +10
...
8. Перевод приложения на два языка (en/ru или en/be) +15
   ...
   - переводится приветствие +3
   - переводится прогноз погоды в т.ч описание погоды (OpenWeatherMap API предоставляет такую возможность) +3
   ...
   - переводятся названия треков в аудиоплеере +3
...
9. Настройки приложения +15  
   ...
   - если источником получения фото указан API, в настройках приложения можно указать тег, для которого API будет присылает фото, например `nature` - фото про природу +3
   - в настройках приложения можно скрыть/отобразить любой из блоков, которые находятся на странице: время, дата, приветствие, цитата дня, прогноз погоды, аудиоплеер, список дел, список ссылок или ваш собственный дополнительный функционал, если он есть +3
...
9. Дополнительный функционал на выбор +15
   - ToDo List - список дел (как в оригинальном приложении) +5
   - список ссылок (как в оригинальном приложении) +5
   - свой собственный дополнительный функционал, по сложности аналогичный предложенным +5
