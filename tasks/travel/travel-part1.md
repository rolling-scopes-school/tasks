...
2. Вёрстка семантическая +20  
   ...
   (Важно! учитывая что не рекомендуется использовать fixed и absolute для `<header>` то будет 5-й элемент `<section>` который будет оберткой для `<header>` и первого `<section>` с контентом. это делается для того получить общий background)
