...
2. Качество кода +130
   - приложение написано на TypeScript. Код полностью покрыт типами +50
      - не используется тип Any
      - используется Everyday Types
      - используется Object Types
      - используется Classes
      - используется Function
   - код разбит на модули +10
   - для сборки кода используется webpack +10
   - используется eslint настроеный для проверки TypeScript, ошибки линтера исправлены +10
