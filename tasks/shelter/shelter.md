...
6. допускается использование CSS-препроцессоров (`Sass`), `normalize.css`
7. не рекомендуется использовать сброс стилей при помощи `reset.css`
8. использовать актуальные библиотеки с набором вспомогательных функций (lodash) не запрещается, но рекомендуется пользоваться чистым JavaScript
