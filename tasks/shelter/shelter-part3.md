...
3. Реализация пагинации на странице `Pets`: **+36** 
   ...
   - при изменении ширины экрана (от 1280px до 320px и обратно), пагинация перестраивается и работает без перезагрузки страницы (страница может оставаться той же, а может быть переключение на первую страницу, при этом сформированный массив - последовательность карточек - не обновляется, сохраняются все остальные требования к пагинации): **+4**
