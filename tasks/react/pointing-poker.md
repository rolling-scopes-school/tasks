...
- особенности командной работы с репозиторием описаны в [Документации курса](TBA)
- название репозитория: **pointing-poker**, название ветки, в которой ведётся разработка - **develop**, ветка **master** пустая, содержит только README.md
...
- добавьте в репозиторий ссылку на созданное API, папку с бекендом, или добавьте в описание pull request ссылку на репозиторий с бекендом
...
- можно использовать bootstrap, css фреймворки, любые js библиотеки, html и css препроцессоры
...
- Наблюдатель - пользователь, который не принимает участия в самой игре, у него нет возможности голосовать или  управлять ходом игры, но он может присоединиться к игре и следитьза ее ходом
- Дилер - пользователь, который настраивает игру, управляет ее ходом. В зависимости от настроек може так же голосовать

### Приветственная страница
  - выполняет функцию страницы входа
  - главная страница приложения содержит:
    - поле для ввода имени. Это имя будет потом использоваться во время игры 
    - кнопку "Начать новую игру". По нажатию на эту кнопку пользователь попадает на страницу с настройками игры 
    - поле для ввода, дропдаун с выбором роли и кнопку "Присоединиться к игре". Если у пользователя есть id уже созданной и активной игры, то он может к ней присоединиться указав id в поле для ввода и нажав на кнопку. После нажатия на кнопку, id должно пройти валидацию(что игра с таким id существует и активна). Если валидация прошла неуспешно, то пользователь остается на приветственной странице и ему показывается сообщение об ошибке, иначе перенаправляется на страницу с игрой(если реализовано лобби, то перенаправляем на страницу лобби)
...
### Настройки
  - в настройках игры у дилера есть возможность указать:
    ...
    - сохранить настройки, чтобы они автоматически применялись при создании новых игр(базовые настройки, не список задач)
    - начать игру
### Страница с игрой
  - эта страница открывается после того, как дилер начал игру
  - где-то на видном месте должна располагаться кнопка "Пригласить". По нажатию на кнопку открывается модальное окно, в котором можно скопировать ссылку на игру и поделиться ею с остальными участниками. Ссылка должна вести на приветственную страницу с презаполненым id игры
  - если реализовано лобби, дилер должен иметь возможность:
    - видеть список участников, ожидающих в лобби
    - запретить участие(в этом случае участник в лобби видит обновленное сообщение, что ему было отказано в доступе и страница автомаически закрывается серез 30 секунд)
  ...
  - поле с игрой содержит следующие элементы:
    - список участников в виде круглых иконок с инициалами внутри(как иконки пользователя в google, jira и т.д.)  и полным именем под иконками. Участники с разной ролью должны визуально выделяться (цветом иконки или разделять по спискам например)
    - список задач:
        ...
        - перемещение между задачами из списка выполняется по клику на элемент списка(или любо другой механизм)
    ...
    - если по настройкам карточки не переворачиваются автоматически, когда проголосовали все, то добавить возможность перевернуть их(например кнопка или переворачивать автоматически по истечению времени на таймере)
    ...
    - механизм чтобы закончить игру. По завершение игры выводится результат оценки и есть возможность сохранить его в виде файла (.csv, .xlsx)
...
При оценивании приложения проверяются все требования, описанные в [Описание функциональных блоков](#описание-функциональных-блоков). Если какие-то из перечисленных требований не выполняются, снимаем часть баллов. В комментарии к оценке необходимо указать какие пункты не выполнены или выполнены частично.
### Вёрстка, дизайн, UI +40
- [ ] вёрстка, дизайн, UI страницы приветствия +10
- [ ] вёрстка, дизайн, UI страницы с настройками +10
- [ ] вёрстка, дизайн, UI страницы с игрой +10
- [ ] оригинальный интересный качественный дизайн приложения +10
### Страница приветствия +30
- [ ] список базовых настроек +10
- [ ] присоединиться к игре +10
- [ ] присоединиться с выбранной ролью +5
...
- [ ] приложение не поддерживает проведение нескольких игр одновременно -100 

## Cross-check
- инструкция по проведению cross-check: https://docs.rs.school/#/cross-check-flow
- форма для проверки задания TBA
- cсылки на лучшие работы добавьте, пожалуйста, в эту [форму](TBA)
