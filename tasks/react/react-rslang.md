...
### Главная страница приложения
  ...
  - главная страница приложения содержит:
    - меню с навигацией по учебнику, ссылками на мини-игры и статистику. Иконка меню отображается на всех страницах приложения
    - описание возможностей и преимуществ приложения
    - небольшое (до 5 минут) видео с демонстрацией работы приложения
...
### Электронный учебник
  - электронный учебник состоит из шести разделов. В каждом разделе 30 страниц. На каждой странице выводится:
...
- #### Настройки
  - в настройках учебника можно указать:
...
- #### Список слов
  - для каждого слова отображается:
    - само слово, его транскрипция, его перевод, часть речи к которой оно относится
    ...
    - если слово добавлено в раздел словаря "Удалённые слова", оно удаляется со страницы учебника
- #### Словарь
  - словарь является частью учебника. В словаре есть разделы "Изучаемые слова", "Сложные слова", "Удалённые слова"
    - в раздел "Изучаемые слова" попадают слова, которые были задействованы в мини-играх
    ...
    - страницы разделов словаря "Сложные слова" и "Удалённые слова" выглядят точно так же, как страницы учебника - формируются списки из 20 слов, если слов больше, создаётся новая страница, на страницах есть мини-игры для повторения слов. Единственное отличие - вместо кнопок, при клике по которым изучаемое слово добавляется в разделы словаря "Сложные слова" или "Удалённые слова", в словаре возле слова отображается кнопка "Восстановить", которая удаляет слово из словаря и восстанавливает его на странице электронного учебника
### Мини-игры "Саванна", "Аудиовызов", "Спринт", "Своя игра"
...
- Часть речи, к которой относится изучаемое слово, возвращает [Skyeng API](https://dictionary.skyeng.ru/doc/api/external).  
- [Пример получения информации о слове](https://github.com/rolling-scopes-school/tasks/blob/master/tasks/rslang/rslang.md#пример-получени-подробной-информации-о-слове)
### Что нужно сделать:
...
### Мини-игры +240 
(максимум +60 баллов за каждую игру)  
Мини-игра может оцениваться в 30, 50 или 60 баллов. При оценке предложенной командой игры, её сложность, интересность, полезность, качество реализации сравнивается с другими мини-играми и оценивается по сравнению с ними.
- [ ] игра в основном соответствует прототипу, является его упрощённой версией +30
- [ ] игра полностью повторяет прототип и детали его работы. Выполняются все перечисленные в задании требования к мини-играм +50
- [ ] игра является улучшенной версией прототипа как с точки зрения внешнего вида и оформления, так и удобства работы. Присутствует дополнительный функционал, улучшающий качество приложения +60
###  Страница статистики +30
- [ ] оформление +10
- [ ] краткосрочная статистика +10 
- [ ] долгосрочная статистика +10
### Бекенд +60
- [ ] при регистрации нового пользователя можно указать его имя и загрузить фото +20
...
- [ ] оригинальная идея, значительный вклад в улучшение качества приложения, заметная полезность, сложность реализации, качество выполнения +10
