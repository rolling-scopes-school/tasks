...
- Также необходимо пригласить ментора, чтобы он мог оценить код
Важное примечание - ментор не должен участвовать в написании кода
- Ментор это помощник в управлении проекта. **Ментор - не кодер**
...
- использование NextJS, Vercel, Gatsby, Remix, и прочие фреймворки не рекомендуются. Использовать create react app. Точка.
...
- TypeScript обязателен (**TypeScript is a must**) 
...
- Вы на курсе React. И мы делаем прежде всего React приложение. Вы можете использовать пиксели и дивы. Если по другому не умеете. Но всё же хотелось бы видеть современное, семантически грамотное приложение с единицами измерения **rem**, а не px. И всё же это рекомендация, а не критерий оценки. 
...
- Логаут - логаутит.
- create new board - открывает модальное окно с формой для создания борды. Требования по этой форме как в остальных. 
  
@TODO

В целом требование к форме и ерор хэндлингу можно взять из основных тасок там оно нормально прописано. С блокировкой кнопки submit, пошаговым сбросом валидации, и т.д.
...
- Требования к модальному окну описаны в тасках предыдущих к курсу.
- Требования к формам как и к формам выше.
...
- пользователь не должен удивляться тому что происходит на странице. при удалении колонки -> ПОДТВЕРЖДЕНИЕ о том, что он теряет все таски или СОХРАНЕНИЕ всех тасков. Возможно здесь будет разумно сделать модальное окно или запретить удаление. Продумайте поведение. 

### Login / Sign up

- Поля форм должны быть реализованы в соответствии с api backend приложения. Должна быть реализована валидация.
- Ошибки со стороны BE -> подробнее в пункте с ошибками
...
- ошибки стилизовать в одном стиле - попапы, модалки и пр. серверные ошибки например в модалке, ошибки формы под формой. Либо это везде pop-up, либо везде модалка
...
Всего возможных баллов за курсов.

Из них:
- по 15 баллов за 6 личных тасков -> **90**.
- **120** баллов за командный таск и Cross-check
- **5** за реализацию CI/CD
- **15** за интервью
...
- За настроенный CI/CD дополнительных 5 баллов от ментора.
