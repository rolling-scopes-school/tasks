...
* Приложение должно быть SPA - Генерация новых страниц должна быть реализована через javascript, перезагрузок страниц не должно быть (Перезагрузки для сброса js состояния, например для сброса состояния игры считаются хаком и штрафуются ).
...
- [ ] **-20** Приложение не SPA (Имеются перезагрузки страницы инициализиуемые напрямую из кода).
