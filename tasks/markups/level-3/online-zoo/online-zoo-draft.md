...
- Крточки животных должны быть интерктивными (хотя бы должен меняться курсор). Нажатие на любую область карточки должно вести на страницу с животным.
...
X. **Footer** (`<footer>` содержит контакты, адрес и изображение):
