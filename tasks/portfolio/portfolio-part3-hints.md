...
Шаблонные строки используются вместо обычных строк в тех случаях, когда внутри строки нам необходимо использовать переменную.  
...
Чтобы не назначать отдельный слушатель каждому элементу с одинаковым или схожим функционалом:
...
- далее можно сделать две проверки:
  ...
  2. Проверить, если в текущий момент перебираемый элемент имеет placeholder, тогда присвоить ему `placeholder` со значением ключа из объекта. Например:
     ...
     ```
     `currentElement.textContent = ''` - на этой строке убираем текст в элементах `input` и `textarea`. Так как мы задали всем элементам одинаковый data-атрибут, в `input`, `textarea` записывается значение из объекта `i18Obj`: и в `placeholder`, и в `textContnet`.
...
- Подсказка. Язык перевода – это аргумент, с которым вызывается функция `getTranslate()`. Ключ, по которому получаем значение, - `dataset.i18n`.
