...
Один из способов оптимизации скорости загрузки страницы на мобильных разрешениях - использование изображений разного размера для компьютера и мобильных устройств.

В данном макете этот способ не используется. Изображения, в том числе фоновые изображения, для всех разрешений одни и те же. То, что в макете в секциях `hero` и `contacts` на разных разрешениях использовались разные картинки, объясняется неопытностью дизайнера. На самом деле фоновые изображения одни и те же.

Больше того, свойства фоновых изображений при изменении ширины макета не меняются, они задаются один раз для фиксированной вёрстки и остаются неизменными вне зависимости от ширины страницы.

Все эти свойства уже указывались в [советах](portfolio-part1-hints.md) к первой части задания. Просто перечитайте их ещё раз, если столкнулись с проблемой несовпадения фона секций при уменьшении ширины страницы.
