...
Задание Museum Stage#1 - расширенная версия одноимённого задания, которое вы выполняли на подготовительном этапе. В ходе его выполнения вам необходимо сверстать landing page Лувра – одного из крупнейших и наиболее популярных художественных музеев мира.
...
- добавлен параллакс
- в `footer` добавились выходные данные сайта - знак копирайта, год создания сайта, ссылка на RS School и ссылка на ваш гитхаб-аккаунт
- небольшие изменения в оформлении: в секции `Tickets` поменялся цвет фона и шрифта, в плеере исчезли кнопки перелистывания вперёд-назад, в секции `Explore` добавились элементы слайдера сравнения изображений 
<!-- шрифт Roboto Thin заменён на Roboto Light  -->
...
- [добавление интерактивности при помощи JavaScript](museum-dom.md) - 2 недели
...
[Макет Museum](https://www.figma.com/file/mUQ3HibvExS1XuMMQ8UZft/Museum---share)  
...
8. js-библиотеки разрешается подключать только для создания слайдера и интерактивной карты. jQuery может подключаться только если она необходима для обеспечения работы js-библиотек
...
- если разрешение экрана 1440рх и больше, проверку проводим в разрешении экрана 100%. Для этого необходимо в дополнении PerfectPixel указать масштаб `1` и вручную выставить макет по верхнему левому углу направляющих, в случае данного макета - по пирамиде - логотипу Лувра.
...
3. Вёрстка соответствует макету +60
   - блок `<header>` +5
   - секция `Welcome` +10
   - секция `Visiting` +5
   - секция `Explore` +5
   ...
   - секция `Contacts` +5
   - блок `<footer>` +5 
4. Форма покупки билетов +25
  - форма открывается при клике по кнопке `Buy Now` в секции `Tickets` и закрывается кликом по иконке с крестиком в верхнем правом углу +2
  - в форме присутствуют следующие элементы: `form`, `input type="date"`, `input type="time"`, `input type="text"`, `input type="email"`, `input type="tel"`, `input type="number"`, `select` +8 
  - вёрстка формы соответствует макету + 15
5. Требования к css + 14
...
7. Интерактивность, реализуемая через js +12
...
Разница между максимальной оценкой за приложение (160 баллов) и максимально возможным количеством баллов за выполнение всех пунктов требований (170 баллов) позволит сгладить возможные ошибки проверяющих в ходе кросс-чека, неточности в описании задания, разное понимание требований задания проверяющим и проверяемым.
