...
- для деплоя используйте `gh-pages` [Как сделать деплой задания из приватного репозитория школы](https://rs.school/ru/docs/private-repository#как-сделать-деплой-задания-из-приватного-репозитория-школы). Файлы для деплоя разместите в ветке `gh-pages`.
...
6. Точки перехода медиа-запросов произвольные в промежутке между проверяемыми значениями
7. При проверке блока `Gallery` проверяем сетку изображений на то, чтобы:
...
Сначала необходимо проверить вёрстку на совпадение с макетом (пункты 1-3), позже на реализацию дополнительных критериев (пункты 4-7).  
