...
В проекте используется семь [Google Street View](https://github.com/rolling-scopes-school/stage1-tasks/blob/museum/README.md#используемые-в-макете-ссылки): шесть из них открываются при клике по карточкам секции `Visiting`, седьмая - при клике по кнопке `Discover the Louvre`  секции `Welcome`.  
...
Когда панорама готова, ссылку на неё необходмо добавить в ваш landing page.  
...
Разместить изображения в три колонки поможет css-свойство `column-count`, указать расстояния между ними по горизонтали и вертикали - свойства `row-gap` и `column-gap`.
...
```
Более современный вариант подключения - использование js-модулей. В данном таске такая задача не ставится, обзорная информация о js-модулях будет в конце этого документа.
...
#### 9. Webpack
Если вы используете js-модули, или добавляете стили с использованием sass (или другого css-препроцессора), или просто хотите научиться новому, попробуйте для сборки проекта использовать Webpack.

Для вас создана отличная сборка https://www.npmjs.com/package/create-rss-app
