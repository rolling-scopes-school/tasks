...
Также необходимо обратить внимание, чтобы содержимое страницы не прилипало к краю окна. На макете у контейнера нет боковых отступов. Внутренние отступы по бокам - `padding-left` и `padding-right` можно добавить блокам `header`, `main` и `footer`. Ещё один возможный вариант решения: указать ширину контейнера за вычетом боковых отступов: `width: calc(100% - 40px);`. 
