...
Изначально студент имеет 280 баллов. Все последующие штрафные баллы отнимаем от 280. Полученный результат и будет являться оценкой.
При оценке cross-check максимальное количество баллов 210.(Студенты проверяют у студентов)

### Базовый уровень

- верстка прототипа приложения не адаптивная и не респонсив – **20 баллов**    
Если верстка адаптивная или респонсив: 
 - верстка ведёт себя некорректно на разрешении 320рх-1440px (в случае адаптивной верстки часть данных может быть скрыта на разрешении меньше дэсктопного) - **10 баллов**  
  
В блоке 2 не отображаются данные, относящиеся к текущему местоположению пользователя – **20 баллов**  
В случае отображения данных:
- Не отображается название населённого пункта, название страны - **5 баллов**
- Не отображается текущая дата: день недели в коротком формате, число, месяц, например, Сб 26 октября - **5 баллов**
- Отображается дата, но не обновляется время: часы и минуты, например, 17:23, обновляется раз в минуту - **3 балла**
- Не отображается температура в текущий промежуток времени - **2 балла**
- Не отображается описание погоды (summary), ощущаемая температура (apparent temperature), скорость ветра(м/с), влажность(%) - **3 балла**
- Не отображается иконка погоды - **2 балла**
  
В блоке 3 не отображается прогноз погоды на три дня, относящийся к текущему местоположению пользователя – **20 баллов**  
В случае отображения данных:
  - день недели не отображается - **5 баллов**
 В случае отображения дня недели:
  - день недели отображается в не полном формате - **2 балла**
  - не отображается температура - **5 баллов**
  - не отображается иконка погоды - **5 баллов**
  
В блоке 4 не отображаются геолокационные данные, относящиеся к текущему местоположению пользователя (широта, долгота, карта) – **20 баллов**  
В случае отображения данных:
- не отображаются координаты населённого пункта: долгота и широта (в градусах и минутах) - **10 баллов**
- Отображаются координаты населённого пункта, но в десятичном виде, а не в градусах и минутах - **5 баллов**
- не отображается карта местности - **10 баллов**

### Средний уровень 

- Не реализован поиск – **60 баллов**   
Если поиск реализован и вводится корректный запрос:
  - не обновляется карта - **10 баллов**
  - не обновляется дата - **10 баллов**
  - не обновляется прогноз на сегодня - **10 баллов**
  - не обновляется прогноз на 3 дня - **10 баллов**
  - не обновляются координаты - **10 баллов**
  - не обновляется фоновое изображение - **10 баллов**  

- При обновлении страницы не меняется фоновое изображение – **10 баллов**
- Отсутствует кнопка смены фонового изображения – **10 баллов**
- Не реализована возможность переключения единиц измерения температуры – **10 баллов**
- Фоновые изображения генерируются без учёта поры года, времени суток, текущей погоды, страны*, названия населённого пункта* (два последних параметра являются необязательными) – **5 баллов**
- Не выполнены требования к коду (оценивает только ментор)
  - html-элементы не генерируются в js - **5 баллов** (оценивает только ментор)
  - js код не разбит на модули - **10 баллов** (оценивает только ментор)
  - не используется webpack - **10 баллов** (оценивает только ментор)
  - не используются editorconfig, eslint, eslint-config-airbnb-base, babel  - **3 балла за каждый** (оценивает только ментор)

### Экстра-уровень 

- Не реализована возможность голосового поиска по названию населённого пункта – **20 баллов**
- Не реализованы юнит-тесты – 2 балла за каждый тест, но не больше **20 баллов** (оценивает только ментор)
- Не реализован перевод текста страницы (en/ru/be) – **20 баллов**
- кнопки, иконки приложения  не анимированы -  **10 баллов**
Кнопки, иконки приложения анимированы:
  - без использования ключевых кадров - **3 балла**
- отсутствует непредусмотреный заданием функционал - **10 баллов**
