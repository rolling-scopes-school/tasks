...
| ----------- | ------------- | ------------- |
| 24.05.2020 23:59 | fancy-weather | fancy-weather |
...
**Максимальный бал за задание: 250 баллов при кросс-чеке / 300 баллов при проверке ментором**
...
- Реализован поиск. Если в поиске вводится корректный запрос, по которому API возвращает результат, все данные на странице, в том числе дата и время, обновляются в соответствии с указанным в поиске населённым пунктом – **50 баллов**
  ...
  - одновременное обновление всех элементов страницы при вводе нового населенного пункта (название по локации приходит быстро, а погода, иконки ее и т.п. медленно) +10
- При обновлении страницы или клике на кнопку для переключения фонового изображения меняется фоновое изображение – **20 баллов**
  - фоновые изображения генерируются с учётом указанного в поле поиска населённого пункта, поры года, времени суток (по желанию можно добавлять и другие параметры поиска). Данные о параметрах запроса фонового изображения для удобства проверки выведите в консоль +10
...
- Реализованы юнит-тесты использующие различные [методы jest](https://jestjs.io/docs/ru/expect) – 2 балла за каждый тест, но не больше **20 баллов** (оценивает только ментор)
...
- Перевод текста страницы (en/ru/be) – **30 баллов**
  ...
  - перевод на белорусский язык названий дней недели и месяцев +5
